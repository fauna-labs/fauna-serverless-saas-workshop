...
def register_tenant(event, context):
    try:
        ...
        provision_tenant_response = __provision_fauna_resources(tenant_details, headers, auth, host, stage_name)
...
def __provision_fauna_resources(tenant_details, headers, auth, host, stage_name):
    try:
        url = ''.join(['https://', host, '/', stage_name, provision_tenant_resource_path])
        logger.info(url)

        # calls the Lambda function that kicks off CodePipeline to run fauna-schema-migrate
        response = requests.post(url, data=json.dumps(tenant_details), auth=auth, headers=headers) 
